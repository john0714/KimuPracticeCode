    ...
    <form action="excelExport.php" method="post">
      ...
      <!-- Ajax cssの為の基本HTML構造設定 -->
        <div class="select-form" id="SF">
          ...
          <div type="button" name="search" id="search"></div>
        </div>
        <div class="button-form" id="BF">
          <div type=button name="modify" id="modify" onclick='DBmodify()' value="" /></div>
          <div type=submit name="ExcelExport"></div>
        </div>
    </div>
      <!-- Ajax table -->
  ...
  <script>
      ...
      function Pageloading() {
        ...
        Attendances_monthlyRef.on('value', function(data) {
          ...
          $.ajax({
            ...
            data: {
              "YearMonth": <?=$year.$month?>, //洗濯した年月 -> JSON形式
    ...
    $(document).on('click', '.search-btn', function() {
      ...
      $.ajax({
        ...
        data: {
          "YearMonth": $("#YearMonth").val(), //洗濯した年月 -> JSON形式
          "Attendances_daily": Attendances_daily,
          "Notevalue": Notevalue
        }, //urlに送る Parameter
        success: function(datas) {
    ...
    $(document).on('click', '.modify-btn', function() {
      ...
      if (buttonText == "修正") { //修正の時
        ...
        $.ajax({
          ...
          url: "timeSheetSearchTable.php", //請求される場所 -> つまり、データを取る場所です
          data: {
            "YearMonth": SelectYM, //選択した年月  -> JSON形式
            "Attendances_daily": Attendances_daily,
            "Notevalue": Notevalue
          }, //urlに送る Parameter
          success: function(datas) {
